...
# ---------- Load models ----------
lstm_model = load_model(f"model/lstm_{ticker}.h5", compile=False)
transformer_model = load_model(
    f"model/transformer_{ticker}.h5",
    compile=False,
...
try:
    ...
    print(f"⚠️ ไม่พบไฟล์ TCN-GRU สำหรับ {ticker}")
    tcn_available = False
